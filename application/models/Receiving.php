...
class Receiving extends CI_Model
{
	...
	public function save($items, $supplier_id, $employee_id, $comment, $reference, $payment_type, $receiving_id = FALSE, $total = 0, $payments = NULL)
	{
		if(count($items) == 0)
		{
			return -1;
		...
		{
			//entry for receiving payments table
			if($receiving_id > 0 && substr( $payment['payment_type'], 0, strlen( $this->lang->line('sales_due') ) ) != $this->lang->line('sales_due'))
			{
			...
			if($receiving_id > 0 && substr( $payment['payment_type'], 0, strlen( $this->lang->line('sales_due') ) ) == $this->lang->line('sales_due'))
			{
				$receivings_payables_data = array(
					'receiving_id' => $receiving_id,
					'amount' => $payment['payment_amount'],
				);
				$this->db->insert('receivings_payables', $receivings_payables_data);
			}
