...
class Sale extends CI_Model
{
	...
	public function get_info($sale_id)
	{
		...
		$this->db->query('CREATE TEMPORARY TABLE IF NOT EXISTS ' . $this->db->dbprefix('sales_payments_temp') .
			'(
				...
				LEFT JOIN ' . $this->db->dbprefix('sales') . ' AS sales
		...
		$this->db->select('
				...
				' . "
				IFNULL($sale_total, $sale_subtotal) AS sale_total,
				IFNULL(MAX(payments.sale_payment_amount), 0) AS amount_paid,
				MAX(sales.change_returned) AS change_returned,
				(IFNULL($sale_total, $sale_subtotal) + MAX(sales.change_returned) - IFNULL(MAX(payments.sale_payment_amount), 0)) AS amount_due,
				" . '
	...
	public function get_sale_info($sale_id)
	{
		...
		if ($sale_data[0]->customer_id != null)
		{
			$this->db->select('
			...
			$this->db->from('people AS people');
			$this->db->where('people.person_id = ' . $sale_data[0]->customer_id);
			
			$customer_data = $this->db->get()->result();
		}
		...
		$this->db->select('
			...
			sales_item.sale_id AS sale_id, 
			sales_item.item_id AS item_id, 
			sales_item.line AS item_line, 
			sales_item.quantity_purchased AS item_quantity,
			sales_item.item_unit_price AS item_retail_price,
			sales_item.discount_percent AS item_discount_percent,
			ROUND(sales_item.item_unit_price * (1 - sales_item.discount_percent / 100), ' . $decimals . ') AS item_sale_price,
			sales_item.item_unit_price * sales_item.quantity_purchased - ROUND((sales_item.item_unit_price * sales_item.quantity_purchased * sales_item.discount_percent / 100), ' . $decimals . ') AS item_subtotal
		');
		
		$this->db->from('sales_items AS sales_item');
		$this->db->from('items AS item');
		$this->db->where('sales_item.sale_id = ' . $sale_id);
		$this->db->where('sales_item.item_id = item.item_id');
		...
		{
			$total_retail_price += $item->item_retail_price * $item->item_quantity;
			$total_discount += round($item->item_retail_price * $item->item_quantity * $item->item_discount_percent / 100, $decimals, PHP_ROUND_HALF_UP);
		}
		$total_final = $total_retail_price - $total_discount;
		...
		if (empty($payments_data))
		{
			...
			$payments_data = $this->db->get()->result();

			$this->sale_lib->set_sales_payments($payments_data);
		}
		...
		{
			$remaining_amount -= $payment->payment_amount;
		}
		...
		$sale_info = array(
			...
			'sale_time' => date($this->config->item('dateformat'), strtotime($sale_data[0]->sale_time)),
			'sale_comment' => $sale_data[0]->comment,
			'total_retail_price' => $total_retail_price,
			'sale_total' => $total_final,
			'total_discount' => $total_discount,
			'remaining_amount' => $remaining_amount,
			'sales_items' => $sales_items,
			'sale_payments' => $payments_data
		);

		if ($sale_data[0]->customer_id != NULL)
		{
			$sale_info['customer_name'] = $customer_data[0]->first_name . ' ' . $customer_data[0]->last_name;
			$sale_info['customer_phone_number'] = $customer_data[0]->phone_number;

		}

		$this->sale_lib->set_sales_info($sale_info);
		return $sale_info;
	}
	...
	public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'sales.sale_time', $order = 'desc', $count_only = FALSE)
	{
		...
		else
		{
			$this->db->select('
					sales.sale_id AS sale_id,
					IFNULL(MAX(receivables.amount), 0) AS amount_receivables,
					MAX(DATE(sales.sale_time)) AS sale_date,
					MAX(sales.sale_time) AS sale_time,
					MAX(sales.invoice_number) AS invoice_number,
					MAX(sales.quote_number) AS quote_number,
		...
		$this->db->join('customers AS customer', 'sales.customer_id = customer.person_id', 'LEFT');
		$this->db->join('sales_payments_temp AS payments', 'sales.sale_id = payments.sale_id', 'LEFT OUTER');
		$this->db->join('sales_receivables AS receivables', 'sales.sale_id = receivables.sale_id', 'LEFT OUTER');
		$this->db->join('sales_items_taxes_temp AS sales_items_taxes',
			'sales_items.sale_id = sales_items_taxes.sale_id AND sales_items.item_id = sales_items_taxes.item_id AND sales_items.line = sales_items_taxes.line',
			'LEFT OUTER');
	...
	/**
	 * Get the receivable summary for the takings (sales/manage) view
	 */
	public function get_receivables_summary($search, $filters)
	{
		// get payment summary
		$this->db->select('SUM(amount) AS receivable_amount');
		$this->db->from('sales AS sales');
		$this->db->join('sales_receivables', 'sales_receivables.sale_id = sales.sale_id');
		$this->db->join('people AS customer_p', 'sales.customer_id = customer_p.person_id', 'LEFT');
		$this->db->join('customers AS customer', 'sales.customer_id = customer.person_id', 'LEFT');
		...
		if($filters['sale_type'] == 'sales')
		{
			$this->db->where('sales.sale_status = ' . COMPLETED . ' AND receivable_amount > 0');
		}
		elseif($filters['sale_type'] == 'quotes')
		{
			$this->db->where('sales.sale_status = ' . SUSPENDED . ' AND sales.quote_number IS NOT NULL');
		...
		$receivables = $this->db->get()->result_array();
	 ...
	 */
	public function save($sale_id, &$sale_status, &$items, $customer_id, $employee_id, $comment, $invoice_number, $work_order_number, $quote_number, $sale_type, $payments, $dinner_table, &$sales_taxes, $change_returned = 0)
	{
		if($sale_id != -1)
		{
		...
		foreach($payments as $payment_id=>$payment)
		{
			...
			if( substr( $payment['payment_type'], 0, strlen( $this->lang->line('sales_due') ) ) == $this->lang->line('sales_due') )
			{
				$sales_payments_data = array(
					'sale_id'		 => $sale_id,
					'amount' => $payment['payment_amount']
				);
				$this->db->insert('sales_receivables', $sales_payments_data);
			}
			else
			{
				$sales_payments_data = array(
		...
		$this->db->query('CREATE TEMPORARY TABLE IF NOT EXISTS ' . $this->db->dbprefix('sales_items_temp') .
			...
			(
				SELECT
					...
					MAX(payments.payment_type) AS payment_type,
					IFNULL(MAX(payments.sale_payment_amount), 0) AS sale_payment_amount,
					' . "
					IFNULL($sale_subtotal, $sale_total) AS subtotal,
					$tax AS tax,
					IFNULL($sale_total, $sale_subtotal) AS total,
	...
	public function edit_sale_item($item_id, $sale_id, $item_line, $quantity)
	{
		...
		{
			if ($item->sale_id == $sale_id && $item->item_id == $item_id && $item->item_line == $item_line)
			{
				$item->item_quantity = $quantity;
				$item->item_subtotal = ($item->item_retail_price * $item->item_quantity) - ROUND(($item->item_retail_price * $item->item_quantity * $item->item_discount_percent / 100), totals_decimals());
				$data = new stdClass;
				$data->isModified = true;
				$extended = (object) array_merge((array)$sales_items[urldecode($line)], (array) $data);
				$sales_items[urldecode($line)] = $extended;
			}
		}
		...
		{
			$total_retail_price += $item->item_retail_price * $item->item_quantity;
			$total_discount += ROUND(($item->item_retail_price * $item->item_quantity * $item->item_discount_percent / 100), total_decimals());
		}
		$total_final = $total_retail_price - $total_discount;

		$payments_data = $sale_info["sale_payments"];
		...
		$sale_info["total_retail_price"] = $total_retail_price;
		$sale_info["sale_total"] = $total_final;
		$sale_info["total_discount"] = $total_discount;
		$sale_info["remaining_amount"] = $remaining_amount;
		$sale_info["sales_items"] = $sales_items;
