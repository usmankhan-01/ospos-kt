...
class Payable extends CI_Model
{
	 ...
	 * Get sale info
	 */
	public function get_info($payable_id)
	{
		$decimals = totals_decimals();

		$sale_total = 'ROUND(SUM(receivings_items.item_unit_price * receivings_items.quantity_purchased * (1 - receivings_items.discount_percent / 100)),' . $decimals . ')';

		// create a temporary table to contain sales totals
		$result = $this->db->query('CREATE TEMPORARY TABLE IF NOT EXISTS ' . $this->db->dbprefix('receivings_totals_temp') .
			' (PRIMARY KEY(receiving_id), INDEX(receiving_id))
			(
				SELECT receivings.receiving_id,' . $sale_total . ' AS total_receiving
				FROM ' . $this->db->dbprefix('receivings') . ' AS receivings, ' .  $this->db->dbprefix('receivings_items') . ' AS receivings_items					
				WHERE ' . $where . ' receivings.receiving_id = receivings_items.receiving_id
		...
		$this->db->select('
					...
					payables.receivings_payables_id AS payable_id, 
					receivings.receiving_time AS receiving_time,
					receivings_totals.total_receiving AS total_receiving,
					payables.amount AS total_due,
					receivings_totals.total_receiving - IFNULL(ROUND(SUM(payments.payment_amount),' . $decimals . '), 0) AS remaining
			');

		$this->db->from('receivings_payables AS payables');
		$this->db->from('receivings as receivings');
		$this->db->from('receivings_totals_temp as receivings_totals');
		$this->db->join('receivings_payments as payments', 'receivings.receiving_id = payments.receiving_id', 'LEFT');
		$where .= " payables.receiving_id = receivings.receiving_id AND receivings_totals.receiving_id = receivings.receiving_id AND payables.receivings_payables_id = " . $payable_id;
	...
	/**
	 * Get number of rows for the takings (receivables/manage) view
	 */
	...
	/**
	 * Get the sales data for the takings (receivables/manage) view
	 */
	public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'receivings.receiving_time', $order = 'desc', $count_only = FALSE)
	{
		...
		// create a temporary table to contain receivings totals
		$result = $this->db->query('CREATE TEMPORARY TABLE IF NOT EXISTS ' . $this->db->dbprefix('receivings_totals_temp') .
			' (PRIMARY KEY(receiving_id), INDEX(receiving_id))
			(
				SELECT receivings.receiving_id,' . $sale_total . ' AS total_receiving
				FROM ' . $this->db->dbprefix('receivings') . ' AS receivings, ' .  $this->db->dbprefix('receivings_items') . ' AS receivings_items					
				WHERE ' . $where . ' AND receivings.receiving_id = receivings_items.receiving_id
				GROUP BY receiving_id
			)'
		);
		...
		if($count_only == TRUE)
		{
			$this->db->select('COUNT(DISTINCT payables.payable_id) as count');
		}
		else
		{
			$this->db->select('
					receivings.receiving_id AS receiving_id, 
					payables.payable_id AS payable_id,
					receivings.receiving_time AS receiving_time,
					CONCAT(supplier.company_name, " ", supplier.agency_name) AS supplier_name,
					receivings_totals.total_receiving AS total_receiving,
					payables.amount AS total_due,
					receivings_totals.total_receiving - IFNULL((ROUND(SUM(payments.payment_amount),' . $decimals . ')), 0) AS remaining
			');
		...
		$where .= " AND payables.receiving_id = receivings.receiving_id  AND receivings_totals.receiving_id = receivings.receiving_id";
		...
		if(!empty($search))
		{
			...
			else
			{
				$this->db->group_start();
					...
					$this->db->like('supplier.company_name', $search);
					// supplier agency name
					$this->db->or_like('supplier.agency_name', $search);
					// supplier company and agency name
					$this->db->or_like('CONCAT(supplier.company_name, " ", supplier.agency_name)', $search);
					// supplier company name
	...
	/**
	 * Get the payment summary for the takings (receivables/manage) view
	 */
	...
	/**
	 * Get the receivable summary for the takings (receivables/manage) view
	 */
	...
	public function addPayment($payments)
	{
		return $this->db->insert('sales_payments', $payments);
	}
	...
	public function row_delete($id, $sale_id, $amount)
	{
