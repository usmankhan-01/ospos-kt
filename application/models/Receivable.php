...
class Receivable extends CI_Model
{
	 ...
	 * Get sale info
	 */
	public function get_info($receivable_id)
	{
		...
		//$sale_total = 'ROUND(SUM(sales_items.item_unit_price * sales_items.quantity_purchased * (1 - sales_items.discount_percent / 100)),' . $decimals . ')';
		$sale_discount = 'ROUND((sales_items.item_unit_price * sales_items.quantity_purchased * sales_items.discount_percent / 100),' . $decimals . ')';
		$sale_total = 'SUM(sales_items.item_unit_price * sales_items.quantity_purchased - ' . $sale_discount . ')';

		// create a temporary table to contain sales totals
		$result = $this->db->query('CREATE TEMPORARY TABLE IF NOT EXISTS ' . $this->db->dbprefix('sales_totals_temp') .
			' (PRIMARY KEY(sale_id), INDEX(sale_id))
			(
				SELECT sales.sale_id,' . $sale_total . ' AS total_sale
				FROM ' . $this->db->dbprefix('sales') . ' AS sales, ' .  $this->db->dbprefix('sales_items') . ' AS sales_items					
				WHERE ' . $where . ' sales.sale_id = sales_items.sale_id
				GROUP BY sale_id
			)'
		);
		...
		$this->db->select('
					...
					receivables.receivable_id AS receivable_id, 
					sales.sale_time AS sale_time,
					sales_totals.total_sale AS total_sale,
					receivables.amount AS total_due,
					sales_totals.total_sale - IFNULL(ROUND(SUM(payments.payment_amount),' . $decimals . '), 0) AS remaining
			');

		$this->db->from('sales_receivables AS receivables');
		$this->db->from('sales as sales');
		$this->db->from('sales_totals_temp as sales_totals');
		$this->db->join('sales_payments as payments', 'sales.sale_id = payments.sale_id', 'LEFT');
		$where .= " receivables.sale_id = sales.sale_id AND sales_totals.sale_id = sales.sale_id AND receivables.receivable_id = " . $receivable_id;
	...
	public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'sales.sale_time', $order = 'desc', $count_only = FALSE)
	{
		...
		$result = $this->db->query('CREATE TEMPORARY TABLE IF NOT EXISTS ' . $this->db->dbprefix('sales_totals_temp') .
			...
			(
				SELECT sales.sale_id,' . $sale_total . ' AS total_sale
				FROM ' . $this->db->dbprefix('sales') . ' AS sales, ' .  $this->db->dbprefix('sales_items') . ' AS sales_items					
				WHERE ' . $where . ' AND sales.sale_id = sales_items.sale_id
				GROUP BY sale_id
			)'
		);
		...
		if($count_only == TRUE)
		{
			$this->db->select('COUNT(DISTINCT receivables.receivable_id) as count');
		}
		else
		{
			$this->db->select('
					...
					receivables.receivable_id AS receivable_id,
					sales.sale_time AS sale_time,
					CONCAT(sale_customer.first_name, " ", sale_customer.last_name) AS customer_name,
					sales_totals.total_sale AS total_sale,
					receivables.amount AS total_due,
					sales_totals.total_sale - IFNULL((ROUND(SUM(payments.payment_amount),' . $decimals . ')), 0) AS remaining
			');
		...
		$this->db->from('sales_totals_temp as sales_totals');
		$this->db->join('people as sale_customer', 'sales.customer_id = sale_customer.person_id', 'LEFT');

		$where .= " AND receivables.sale_id = sales.sale_id  AND sales_totals.sale_id = sales.sale_id";
