...
class Receiving_lib
{
	...
	public function add_item($item_id, $quantity = 1, $item_location = NULL, $discount = 0, $price = NULL, $description = NULL, $serialnumber = NULL, $receiving_quantity = NULL, $include_deleted = FALSE)
	{
		...
		// first check by bracode
		if($this->CI->Item->item_exists($item_id, $include_deleted))
		{
		...
		else if(!$this->CI->Item->exists($item_id, $include_deleted))
		{
			return FALSE;
		}
	...
	// Multiple Payments
	public function get_receiving_payments()
	{
		if(!$this->CI->session->userdata('receiving_payments'))
		{
			$this->set_receiving_payments(array());
		}
	...
	// Multiple Payments
	public function set_receiving_payments($payments_data)
	{
		$this->CI->session->set_userdata('receiving_payments', $payments_data);
	}

	// Multiple Payments
	public function add_receiving_payment($payment_id, $payment_amount)
	{
		$payments = $this->get_receiving_payments();
		...
		$this->set_receiving_payments($payments);
	}

	// Multiple Payments
	public function edit_receiving_payment($payment_id, $payment_amount)
	{
		$payments = $this->get_receiving_payments();
		if(isset($payments[$payment_id]))
		{
	...
	// Multiple Payments
	public function delete_receiving_payment($payment_id)
	{
		$payments = $this->get_receiving_payments();
	...
	// Multiple Payments
	public function empty_receiving_payments()
	{
		$this->CI->session->unset_userdata('receiving_payments');
	}

	// Multiple Payments
	public function get_receiving_payments_total()
	{
		$subtotal = 0;
		$this->reset_cash_flags();
		foreach($this->get_receiving_payments() as $payments)
		{
