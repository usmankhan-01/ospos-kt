...
class Sale_lib
{
	...
	public function return_entire_sale($receipt_sale_id)
	{
		...
		{
			$this->add_item($row->item_number, -$row->quantity_purchased, $row->item_location, $row->discount_percent, PRICE_MODE_STANDARD, NULL, NULL, $row->item_unit_price, $row->description, $row->serialnumber, TRUE);
		}
	...
	public function copy_entire_sale($sale_id)
	{
		...
		foreach($this->CI->Sale->get_sale_items_ordered($sale_id)->result() as $row)
		{
			if ($row->item_number != NULL && $row->item_number != '')
				$this->add_item($row->item_number, $row->quantity_purchased, $row->item_location, $row->discount_percent, PRICE_MODE_STANDARD, NULL, NULL, $row->item_unit_price, $row->description, $row->serialnumber, TRUE, $row->print_option);
			else
	...
	// Multiple Payments
	public function get_sales_payments()
	{
		if(!$this->CI->session->userdata('sales_payments_data'))
		{
			$this->set_sales_payments(array());
		}

		return $this->CI->session->userdata('sales_payments_data');
	}

	// Multiple Payments
	public function set_sales_payments($payments_data)
	{
		$this->CI->session->set_userdata('sales_payments_data', $payments_data);
	}

	// Multiple Payments
	public function add_sales_payment($payment_id, $payment_amount)
	{
		$payments = $this->get_sales_payments();
		if(isset($payments[$payment_id]))
		{
			//payment_method already exists, add to payment_amount
			$payments[$payment_id]['payment_amount'] = bcadd($payments[$payment_id]['payment_amount'], $payment_amount);
		...
		$this->set_sales_payments($payments);
	}

	// Multiple Payments
	public function edit_sales_payment($payment_id, $payment_amount)
	{
		$payments = $this->get_sales_payments();
		if(isset($payments[$payment_id]))
		{
	...
	// Multiple Payments
	public function delete_sales_payment($index)
	{
		$payments = $this->get_sales_payments();
		$data = new stdClass;
		$data->isDelete = true;
		$extended = (object) array_merge((array)$payments[urldecode($index)], (array) $data);
		$payments[urldecode($index)] = $extended;
		//unset($payments[urldecode($index)]);
		$this->set_sales_payments($payments);
	}

	// Multiple Payments
	public function empty_sales_payments()
	{
		$this->CI->session->unset_userdata('sales_payments_data');
	}

	// Multiple Payments
	public function get_sales_payments_total()
	{
		$subtotal = 0;
		$this->reset_cash_flags();
		foreach($this->get_sales_payments() as $payments)
		{
			$subtotal = bcadd($payments['payment_amount'], $subtotal);
			if($this->CI->session->userdata('cash_rounding') && $this->CI->lang->line('sales_cash') != $payments['payment_type'])
			{
	...
	public function get_sales_totals()
	{
		...
		$payment_total = $this->get_sales_payments_total();
		$totals['payment_total'] = $payment_total;
	...
	//SALES INFO RELATED FUNCTIONS
	public function get_sales_info()
	{
		if(!$this->CI->session->userdata('sales_info_data'))
		{
			$this->set_sales_info(array());
		}

		return $this->CI->session->userdata('sales_info_data');
	}

	public function set_sales_info($sales_info)
	{
		$this->CI->session->set_userdata('sales_info_data', $sales_info);
	}

	public function delete_sales_info($index)
	{
		$payments = $this->get_sales_payments();
		$data = new stdClass;
		$data->isDelete = true;
	...
	public function empty_sales_info()
	{
		$this->CI->session->unset_userdata('sales_info_data');
	}

	public function delete_sales_item($item_id, $sale_id, $item_line)
	{
		$sale_info = $this->get_sales_info();
		...
		foreach($sales_items as $item)
		{
			...
			$total_discount += $item->item_retail_price * $item->item_quantity * $item->item_discount_percent / 100;
		}
