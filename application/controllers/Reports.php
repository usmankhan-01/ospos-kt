...
class Reports extends Secure_Controller
{
	...
	public function summary_items($start_date, $end_date, $sale_type, $location_id = 'all')
	{
		...
		$data = array(
			...
			'mode' => $sale_type,
			'stock_locations' => array_reverse($stock_locations, TRUE),
			'sale_type_options' => $this->get_sale_type_options("summary_items"),
			'start_date' => $start_date,
			'end_date' => $end_date
		);
	...
	public function summary_cashs($start_date, $end_date, $sale_type, $location_id)
	{
		$inputs = array('start_date' => $start_date, 'end_date' => $end_date, 'sale_type' => $sale_type, 'location_id' => $location_id);

		$this->load->model('reports/Summary_cashs');
		$model = $this->Summary_cashs;
		...
		foreach($report_data as $row)
		{
			$tabular_data[] = $this->xss_clean(array(
				...
				'payment_type' => $row['payment_type'],
				'amount' => to_currency($row['payment_amount']),
				'action' => anchor("receivables/view/".$row['sale_id'], 'Sale',	array('class' => 'modal-dlg modal-dlg-wide print_hide btn btn-primary btn-xs', 'data-btn-save' => 'Save Changes', 'title' => 'test')),
		...
		$data = array(
			'title' => $this->lang->line('reports_cashs_summary_report'),
			'subtitle' => $this->_get_subtitle_report(array('start_date' => $start_date, 'end_date' => $end_date)),
			'headers' => $this->xss_clean($model->getDataColumns()),
			'data' => $tabular_data,
			'summary_data' => $summary,
	...
	//CUSTOM: Input for reports. (see routes.php to see that all graphical summary reports route here)
	public function custom_date_input()
	{
		$data = array();
		$stock_locations = $data = $this->xss_clean($this->Stock_location->get_allowed_locations('sales'));
		$stock_locations['all'] = $this->lang->line('reports_all');
		$data['stock_locations'] = 'all';
		$data['mode'] = 'sale';
		$data['start_date'] = date("Y-m-d");
		$data['end_date'] = date("Y-m-d");

		$this->load->view('reports/custom_date_input', $data);
	}
	...
	public function get_sale_type_options($report_name = "")
	{
		$sale_type_options = array();
		$sale_type_options['complete'] = $this->lang->line('reports_complete');
		$sale_type_options['paid'] = $this->lang->line('reports_paid');
		$sale_type_options['sales'] = $this->lang->line('reports_completed_sales');
		if($report_name == "summary_items")
		{
			$sale_type_options['due'] = $this->lang->line('reports_due');
		}
