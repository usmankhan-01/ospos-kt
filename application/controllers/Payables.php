...
class Payables extends Secure_Controller
{
	public function __construct()
	{
		parent::__construct('payables');
	...
	public function index()
	{
		...
		$this->db->update('receiving_payables');
		...
		{
			$data['table_headers'] = get_payables_manage_table_headers();
		...
		$this->load->view("payables/manage", $data);
	}
	...
	public function search()
	{
		...
		$payables = $this->Payable->search($search, $filters, $limit, $offset, $sort, $order);
		$total_rows = $this->Payable->get_found_rows($search, $filters);

		$data_rows = array();
		$index = 1;
		foreach($payables->result() as $payable)
		{
			$data_rows[] = $this->xss_clean(get_payables_data_row($payable, $offset + $index));
			$index++;
		}

		if($total_rows > 0)
		{
			$data_rows[] = $this->xss_clean(get_payables_data_last_row($payables));
		}

		echo json_encode(array('total' => $total_rows, 'rows' => $data_rows));
	}

	public function edit($payable_id)
	{
		$data = array();
		...
		$payable_info = $this->xss_clean($this->Payable->get_info($payable_id)->row_array());
		$data['payable_amount'] = $payable_info['remaining'];
		$data['payable_id'] = $payable_info['payable_id'];
		$data['receiving_id'] = $payable_info['receiving_id'];
		$data['payable_info'] = $payable_info;

		$this->load->view('payables/form', $data);
	}
	...
	public function save($receivable_id = -1)
	{
		...
		{
			echo json_encode(array('success' => TRUE, 'message' => $this->lang->line('payables_successfully_updated'), 'id' => $receivable_id));
		}
		else
		{
			echo json_encode(array('success' => FALSE, 'message' => $this->lang->line('payables_unsuccessfully_updated'), 'id' => $receivable_id));
		}
	}

	public function view($receiving_id)
	{
		$this->receiving_lib->empty_receivings_payments();

		$data = array();

		$receiving_info = $this->Receiving->get_receiving_info($receiving_id);

		$data["receiving_info"] = $receiving_info;
		$data["recevings_payments"] = $receiving_info["recevings_payments"];
		$data["receivings_items"] = $receiving_info["receivings_items"];
		$this->load->view('receivings/receiving_info', $data);
	}
	...
	public function save_payments()
	{
		...
		//saving remaining amount in payables table
		$amount = array('amount' => $sale_info["remaining_amount"]); 
		$this->db->where('sale_id', $sale_info["sale_id"]);
		$this->db->update('receiving_payables', $amount);
	 
		$this->output
		->set_content_type("application/json")
		->set_output(json_encode(array('status'=>true, 'redirect'=>base_url('payables') )));
	}
