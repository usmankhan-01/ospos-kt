...
class Sales extends Secure_Controller
{
	...
	public function search()
	{
		...
		$receivables = $this->Sale->get_receivables_summary($search, $filters);
		$payment_summary = $this->xss_clean(get_sales_manage_payments_receivables_summary($payments, $sales, $receivables));
	...
	public function complete()
	{
		...
		if($this->sale_lib->is_invoice_mode() || $pos_invoice)
		{
			...
			else
			{
				...
				$data['sale_id_num'] = $this->Sale->save($sale_id, $data['sale_status'], $data['cart'], $customer_id, $employee_id, $data['comments'], $invoice_number, $work_order_number, $quote_number, $sale_type, $data['payments'], $data['dinner_table'], $data['taxes'], $data['amount_change']);
